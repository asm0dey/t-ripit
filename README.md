...
#### Run
```
mvn -q exec:java -Dexec.args="--start-date 2023-01-01 --until-date 2024-12-31 --out flights.csv --headed"
```
Options:
...
Alternative run (after packaging) — note this project uses the exec plugin primarily:
```
mvn -q -DskipTests package
# Then run via exec (preferred as it sets the correct classpath):
mvn -q exec:java -Dexec.args="--out flights.csv"
```
